...
$('main > section, body > section').each((i, el) => {
    let id = $(el).attr('id');
    const htmlObj = $.html(el);
    if (id) {
        map[id] = htmlObj;
    } else {
        const text = $(el).text();
        if (text.includes('Hablar otro idioma cambia')) {
            map['oportunidades'] = htmlObj;
        } else if (text.includes('Clases para jóvenes')) {
            map['banner-jovenes'] = htmlObj;
        } else {
            map['unmapped_' + i] = htmlObj;
        }
    }
    $(el).remove();
});
...
// append remaining
Object.values(map).forEach(htmlObj => {
    $('main').append(htmlObj);
});

// FIX NAV:
const newNav = `
        <div class="flex flex-row items-center justify-between w-full px-8 py-3 max-w-7xl mx-auto">
            <!-- Logo - Start -->
            ...
            <div class="hidden lg:flex flex-none items-center justify-center gap-8 font-black text-sm tracking-wide">
                ...
                <a class="nav-link text-on-surface-variant hover:text-primary transition-colors duration-300" href="#reviews">Reseñas</a>
...
$('label[for="reg-lang"]').text('Programa de interés');
