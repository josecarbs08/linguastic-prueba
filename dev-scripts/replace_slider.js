...
const startIdx = html.indexOf('<section id="languages"');
const endStr = '</section>';
const afterSectionIdx = html.indexOf(endStr, startIdx);

// The regex might not match the entire section if there's nested sections, but here there shouldn't be. 
// Let's use a safer approach for nested tags, or just a simple replace since we know the structure.
// Actually, let's just find the next <section> tag, and the section closes right before it.
let endIdx = html.indexOf('<section', startIdx + 1);
if (endIdx === -1) endIdx = html.indexOf('</main>');

const partToRemove = html.substring(startIdx, endIdx);

// Make sure the last part of partToRemove is actually </section> and whitespace
const actualEndIdx = html.lastIndexOf('</section>', endIdx) + '</section>'.length;

html = html.substring(0, startIdx) + newSliderHtml + '\n\n' + html.substring(actualEndIdx);
