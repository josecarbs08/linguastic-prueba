...
$('main > section').each((i, el) => {
    ...
    if (id) {
        ...
        if (text.includes('Hablar otro idioma cambia')) {
            map['oportunidades'] = el;
        } else if (text.includes('Clases para jóvenes')) {
            map['banner-jovenes'] = el;
        } else {
            map['unmapped_' + i] = el;
        }
    }
...
// Remove any remaining sections in body
$('body > section').each((i, el) => {
    let id = $(el).attr('id');
    if (id) {
        if (id === 'location' || id === 'faq' || id === 'reviews') {
            map[id] = el;
            $(el).detach();
        }
    }
...
// Update dropdowns / hidden forms in case they have German/French options
// They requested English-only.
